...
const _TtsSession = class _TtsSession {
  constructor({ voiceId, progress, logger, wasmPaths, speakerId }) {
    ...
    if (_TtsSession._instance) {
      ...
      __privateSet(
        _TtsSession._instance,
        _progressCallback,
        progress ?? __privateGet(_TtsSession._instance, _progressCallback)
      )
    ...
    ;(_a = __privateGet(this, _logger)) == null
      ? void 0
      : _a.call(this, `Loaded WASMPaths at: ${JSON.stringify(__privateGet(this, _wasmPaths))}`)
  ...
  async init() {
    ...
    const modelBlob = await getBlob(`${HF_BASE}/${path}`, __privateGet(this, _progressCallback))
    __privateSet(
      this,
      _ortSession,
      await __privateGet(this, _ort).InferenceSession.create(await modelBlob.arrayBuffer())
    )
  }
  async predict(text) {
    ...
    const speakerId = __privateGet(this, _speakerId)
    ...
    const feeds = {
      ...
      scales: new (__privateGet(this, _ort).Tensor)('float32', [noiseScale, lengthScale, noiseW]),
    }
