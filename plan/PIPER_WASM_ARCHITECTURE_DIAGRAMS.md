# 🎨 Diagrammes d'Architecture : Intégration Piper-WASM

**Projet** : Répét  
**Branche** : `piper-wasm`  
**Version** : 1.0

---

## 📊 Vue d'Ensemble du Système

```
┌─────────────────────────────────────────────────────────────────┐
│                         RÉPÉT APPLICATION                        │
│                     (React 18 + TypeScript)                      │
└─────────────────────────────────────────────────────────────────┘
                                 │
                    ┌────────────┴────────────┐
                    │                         │
         ┌──────────▼──────────┐   ┌─────────▼──────────┐
         │   ReaderScreen      │   │  SettingsScreen    │
         │  (Lecture Texte)    │   │   (Configuration)  │
         └──────────┬──────────┘   └─────────┬──────────┘
                    │                        │
                    │         ┌──────────────┘
                    │         │
         ┌──────────▼─────────▼──────────┐
         │   TTSProviderManager          │
         │   (Orchestrateur Central)     │
         │                                │
         │  - Sélection provider actif   │
         │  - Routing des commandes      │
         │  - Gestion du cache           │
         └──────────┬─────────────────────┘
                    │
        ┌───────────┴───────────┐
        │                       │
┌───────▼──────────┐   ┌────────▼─────────┐
│ WebSpeechProvider│   │ PiperWASMProvider│
│                  │   │                  │
│ ┌──────────────┐ │   │ ┌──────────────┐ │
│ │ Web Speech   │ │   │ │ Piper WASM   │ │
│ │ API (native) │ │   │ │ Module       │ │
│ └──────────────┘ │   │ └──────────────┘ │
│                  │   │                  │
│ ┌──────────────┐ │   │ ┌──────────────┐ │
│ │ VoiceManager │ │   │ │ Model Cache  │ │
│ │ TTSEngine    │ │   │ │ (IndexedDB)  │ │
│ └──────────────┘ │   │ └──────────────┘ │
└──────────────────┘   └──────────────────┘
```

---

## 🔄 Flux de Données : Lecture Audio

### Scénario 1 : Première Lecture avec Piper

```
┌─────────────┐
│ Utilisateur │
│ clique Play │
└──────┬──────┘
       │
       ▼
┌──────────────────────────────────────┐
│ ReaderScreen                         │
│  → getSelectedVoice()                │
│  → ttsProviderManager.speak(...)     │
└──────┬───────────────────────────────┘
       │
       ▼
┌────────────────────────────────────────────────┐
│ TTSProviderManager                             │
│  1. Vérifie provider actif (piper-wasm)       │
│  2. Génère cache key (hash text + options)    │
│  3. Vérifie AudioCacheService                 │
│  4. Cache MISS → délègue à PiperWASMProvider  │
└──────┬─────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────┐
│ PiperWASMProvider                            │
│  1. Vérifie si modèle chargé en mémoire      │
│  2. NON → downloadAndLoadModel()             │
│     ┌────────────────────────────────────┐   │
│     │ Téléchargement Modèle              │   │
│     │ - Fetch depuis CDN                 │   │
│     │ - Progress tracking (events)       │   │
│     │ - Store dans loadedModels Map      │   │
│     └────────────────────────────────────┘   │
│  3. piperModule.synthesize(text, options)    │
│  4. Génère AudioBuffer (WAV)                 │
│  5. Crée Blob → URL.createObjectURL()        │
│  6. Cache dans AudioCacheService             │
│  7. Retourne HTMLAudioElement                │
└──────┬───────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────┐
│ AudioCacheService                    │
│  → Stocke dans IndexedDB             │
│  → Key: audio_[hash]                 │
│  → Value: { blob, metadata }         │
└──────┬───────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────┐
│ ReaderScreen                         │
│  → audio.play()                      │
│  → events.onStart()                  │
│  → events.onEnd() (quand terminé)    │
└──────────────────────────────────────┘
```

### Scénario 2 : Lecture Suivante (Cache HIT)

```
┌─────────────┐
│ Utilisateur │
│ clique Play │
└──────┬──────┘
       │
       ▼
┌──────────────────────────────────────┐
│ ReaderScreen                         │
│  → ttsProviderManager.speak(...)     │
└──────┬───────────────────────────────┘
       │
       ▼
┌────────────────────────────────────────────────┐
│ TTSProviderManager                             │
│  1. Génère cache key                           │
│  2. Vérifie AudioCacheService                  │
│  3. Cache HIT ✅                               │
│  4. Retourne audio directement (< 100ms)       │
└──────┬─────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────┐
│ ReaderScreen                         │
│  → audio.play() (INSTANTANÉ)         │
└──────────────────────────────────────┘
```

---

## 🏗️ Architecture des Providers

### Interface TTSProvider (Contrat)

```
┌───────────────────────────────────────────────┐
│              <<interface>>                    │
│              TTSProvider                      │
├───────────────────────────────────────────────┤
│ + type: TTSProviderType                       │
│ + name: string                                │
├───────────────────────────────────────────────┤
│ + initialize(): Promise<void>                 │
│ + checkAvailability(): Promise<Availability>  │
│ + getVoices(): Promise<VoiceDescriptor[]>     │
│ + synthesize(...): Promise<SynthesisResult>   │
│ + stop(): void                                │
│ + pause?(): void                              │
│ + resume?(): void                             │
│ + dispose(): Promise<void>                    │
└───────────────────────────────────────────────┘
                      △
                      │
         ┌────────────┴────────────┐
         │                         │
┌────────┴─────────┐     ┌─────────┴────────┐
│ WebSpeechProvider│     │ PiperWASMProvider│
├──────────────────┤     ├──────────────────┤
│ - engine         │     │ - piperModule    │
│ - synth          │     │ - loadedModels   │
│                  │     │ - currentAudio   │
├──────────────────┤     ├──────────────────┤
│ + initialize()   │     │ + initialize()   │
│ + synthesize()   │     │ + synthesize()   │
│ + stop()         │     │ + stop()         │
│ + pause()        │     │ + pause()        │
│ + resume()       │     │ + resume()       │
└──────────────────┘     └──────────────────┘
```

---

## 🎭 Flux d'Assignation de Voix par Genre

### Principe : Maximiser la Diversité Vocale

```
┌─────────────────────────────────────────────────────────────┐
│ Pièce : 4 personnages                                       │
│  - JULIETTE (Femme) ──┐                                     │
│  - CLAIRE (Femme)   ──┤                                     │
│  - ROMÉO (Homme)    ──┤                                     │
│  - MARC (Homme)     ──┘                                     │
└─────────────────────────────────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────────┐
│ PlayDetailScreen : "Voix des personnages"                   │
│                                                             │
│ Utilisateur définit le genre de chaque personnage :        │
│  ♀ JULIETTE  → Femme  [settings.characterVoices]          │
│  ♀ CLAIRE    → Femme                                       │
│  ♂ ROMÉO     → Homme                                       │
│  ♂ MARC      → Homme                                       │
└─────────────────────────────────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────────┐
│ Lecture : PlayScreen                                        │
│  → Pour chaque réplique de personnage                       │
│  → TTSProviderManager.speak(...)                            │
└──────────────┬──────────────────────────────────────────────┘
               │
               ▼
┌──────────────────────────────────────────────────────────────┐
│ PiperWASMProvider.selectVoiceForCharacter(id, gender)       │
│                                                              │
│ JULIETTE (female) :                                         │
│  ├─ Voix féminines disponibles : [Siwis, UPMC]             │
│  ├─ Compteur utilisation : Siwis=0, UPMC=0                 │
│  └─ Sélection : Siwis (la moins utilisée)                  │
│     Compteur : Siwis=1                                      │
│                                                              │
│ CLAIRE (female) :                                           │
│  ├─ Voix féminines disponibles : [Siwis, UPMC]             │
│  ├─ Compteur utilisation : Siwis=1, UPMC=0                 │
│  └─ Sélection : UPMC (la moins utilisée) ✅ DIFFÉRENTE      │
│     Compteur : UPMC=1                                       │
│                                                              │
│ ROMÉO (male) :                                              │
│  ├─ Voix masculines disponibles : [Tom, Gilles]            │
│  ├─ Compteur utilisation : Tom=0, Gilles=0                 │
│  └─ Sélection : Tom (la moins utilisée)                    │
│     Compteur : Tom=1                                        │
│                                                              │
│ MARC (male) :                                               │
│  ├─ Voix masculines disponibles : [Tom, Gilles]            │
│  ├─ Compteur utilisation : Tom=1, Gilles=0                 │
│  └─ Sélection : Gilles (la moins utilisée) ✅ DIFFÉRENTE    │
│     Compteur : Gilles=1                                     │
│                                                              │
│ Cache : voiceAssignments Map                                │
│  JULIETTE → fr_FR-siwis-medium                             │
│  CLAIRE   → fr_FR-upmc-medium                              │
│  ROMÉO    → fr_FR-tom-medium                               │
│  MARC     → fr_FR-gilles-medium                            │
└──────────────────────────────────────────────────────────────┘
               │
               ▼
┌──────────────────────────────────────────────────────────────┐
│ Résultat : 4 VOIX DIFFÉRENTES                               │
│  ✅ Diversité maximale                                       │
│  ✅ Respect des genres                                       │
│  ✅ Cohérence (même voix pour même personnage)              │
└──────────────────────────────────────────────────────────────┘
```

### Cas Limite : Plus de Personnages que de Voix

```
┌─────────────────────────────────────────────────────────────┐
│ Pièce : 5 personnages féminins                              │
│ Voix disponibles : 2 féminines (Siwis, UPMC)               │
└─────────────────────────────────────────────────────────────┘
                          │
                          ▼
┌──────────────────────────────────────────────────────────────┐
│ Assignation avec Rotation Équitable                         │
│                                                              │
│ JULIETTE → Siwis  (usage: 0 → 1)                           │
│ CLAIRE   → UPMC   (usage: 0 → 1)                           │
│ OPHÉLIE  → Siwis  (usage: 1 → 2) ⟲ Rotation                │
│ PORTIA   → UPMC   (usage: 1 → 2) ⟲ Rotation                │
│ DESDÉMONE→ Siwis  (usage: 2 → 3) ⟲ Rotation                │
│                                                              │
│ Résultat : Usage équilibré (Siwis=3, UPMC=2)               │
└──────────────────────────────────────────────────────────────┘
```

### Algorithme de Sélection

```typescript
selectVoiceForCharacter(characterId: string, gender: Gender): string {
  // 1. Vérifier le cache (cohérence)
  if (this.voiceAssignments.has(characterId)) {
    return this.voiceAssignments.get(characterId)!;
  }
  
  // 2. Filtrer par genre
  const modelsOfGender = PIPER_MODELS.filter(m => m.gender === gender);
  
  // 3. Trouver la voix la moins utilisée
  let selectedModel = modelsOfGender[0];
  let minUsage = this.voiceUsageCount.get(selectedModel.id) || 0;
  
  for (const model of modelsOfGender) {
    const usage = this.voiceUsageCount.get(model.id) || 0;
    if (usage < minUsage) {
      minUsage = usage;
      selectedModel = model;
    }
  }
  
  // 4. Enregistrer et incrémenter
  this.voiceAssignments.set(characterId, selectedModel.id);
  this.voiceUsageCount.set(selectedModel.id, minUsage + 1);
  
  return selectedModel.id;
}
```

---

## 🗄️ Structure de Stockage

### IndexedDB : AudioCache

```
Database: RepetAudioCache (v1)
│
└─── ObjectStore: audioCache
     │
     ├─── Key: "audio_abc123" (cache key)
     │    Value: {
     │      key: "audio_abc123",
     │      audioBlob: Blob (WAV),
     │      createdAt: 1704067200000,
     │      accessCount: 5,
     │      lastAccess: 1704153600000
     │    }
     │
     ├─── Key: "audio_def456"
     │    Value: { ... }
     │
     └─── Key: "audio_ghi789"
          Value: { ... }
```

### Memory : Voice Assignments (In-Memory Cache)

```
PiperWASMProvider - Instance Memory

voiceAssignments: Map<string, string>
├── "char_juliette_123" → "fr_FR-siwis-medium"
├── "char_claire_456"   → "fr_FR-upmc-medium"
├── "char_romeo_789"    → "fr_FR-tom-medium"
└── "char_marc_012"     → "fr_FR-gilles-medium"

voiceUsageCount: Map<string, number>
├── "fr_FR-siwis-medium" → 1
├── "fr_FR-upmc-medium"  → 1
├── "fr_FR-tom-medium"   → 1
└── "fr_FR-gilles-medium"→ 1
```

### LocalStorage : Configuration TTS

```
Key: "repet-tts-config"
Value: {
  "state": {
    "selectedProvider": "piper-wasm",
    // Futures options...
  },
  "version": 1
}

Key: "repet-play-settings-[playId]"
Value: {
  "state": {
    "characterVoices": {
      "char_juliette_123": "female",
      "char_claire_456": "female",
      "char_romeo_789": "male",
      "char_marc_012": "male"
    },
    // ... autres settings
  },
  "version": 1
}
```

---

## 🔀 Flux de Changement de Provider

```
┌─────────────────────┐
│ Utilisateur         │
│ sélectionne "Piper" │
│ dans Settings       │
└──────┬──────────────┘
       │
       ▼
┌────────────────────────────────────┐
│ TTSEngineSelector                  │
│  → handleProviderChange('piper')   │
└──────┬─────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────┐
│ ttsProviderManager                           │
│  1. stop() sur provider actuel               │
│  2. activeProvider = null                    │
│  3. provider = providers.get('piper-wasm')   │
│  4. provider.initialize()                    │
│  5. activeProvider = provider                │
└──────┬───────────────────────────────────────┘
       │
       ▼
┌────────────────────────────────────┐
│ useTTSConfigStore                  │
│  → setProvider('piper-wasm')       │
│  → Persist dans localStorage       │
└──────┬─────────────────────────────┘
       │
       ▼
┌────────────────────────────────────┐
│ App.tsx (useEffect)                │
│  → Détecte changement              │
│  → Réinitialise TTS                │
└────────────────────────────────────┘
```

---

## 🎨 Composants UI

### SettingsScreen → TTSEngineSelector

```
┌────────────────────────────────────────────────┐
│ SettingsScreen                                 │
│ ┌────────────────────────────────────────────┐ │
│ │ 📢 Synthèse Vocale                         │ │
│ │                                            │ │
│ │ ┌────────────────────────────────────────┐ │ │
│ │ │ TTSEngineSelector                      │ │ │
│ │ │                                        │ │ │
│ │ │ Moteur de génération des voix          │ │ │
│ │ │                                        │ │ │
│ │ │ ○ Voix Système (Navigateur)           │ │ │
│ │ │   Utilise les voix système...          │ │ │
│ │ │                                        │ │ │
│ │ │ ● Piper (Voix Hors-ligne) [Recommandé]│ │ │
│ │ │   Voix de haute qualité...             │ │ │
│ │ └────────────────────────────────────────┘ │ │
│ │                                            │ │
│ │ ┌────────────────────────────────────────┐ │ │
│ │ │ VoiceSelector (existant)               │ │ │
│ │ │ Choix de la voix : [Siwis Femme ▼]    │ │ │
│ │ └────────────────────────────────────────┘ │ │
│ │                                            │ │
│ │ ┌────────────────────────────────────────┐ │ │
│ │ │ PiperModelManager (optionnel)          │ │ │
│ │ │ Espace utilisé : 12 MB                 │ │ │
│ │ │ [Vider le cache]                       │ │ │
│ │ └────────────────────────────────────────┘ │ │
│ └────────────────────────────────────────────┘ │
└────────────────────────────────────────────────┘
```

---

## 📦 Dépendances et Modules

### Avant (Existant)

```
src/core/tts/
├── engine.ts          → TTSEngine (Web Speech API)
├── voice-manager.ts   → VoiceManager (système)
├── queue.ts           → SpeechQueue
├── types.ts           → Types TTS
└── readingModes.ts    → Modes de lecture
```

### Après (Piper-WASM)

```
src/core/tts/
├── provider/           ┌─────────────────┐
│   ├── types.ts       │ NOUVEAU         │
│   ├── TTSProviderManager.ts  │         │
│   ├── WebSpeechProvider.ts   │         │
│   ├── PiperWASMProvider.ts   │         │
│   └── AudioCacheService.ts   │         │
│                      └─────────────────┘
├── engine.ts          (inchangé)
├── voice-manager.ts   (inchangé)
├── queue.ts           (inchangé)
├── types.ts           (inchangé)
├── readingModes.ts    (inchangé)
└── index.ts           (exports modifiés)
```

---

## 🔄 Cycle de Vie d'un Provider

### Initialisation App

```
App Mount
   │
   ▼
┌─────────────────────────────────────┐
│ useTTSConfigStore                   │
│  → selectedProvider = 'piper-wasm'  │
│    (depuis localStorage)            │
└──────┬──────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────┐
│ ttsProviderManager.initialize('piper-wasm')  │
│                                              │
│ 1. providers.get('piper-wasm')              │
│    ┌──────────────────────────────────┐     │
│    │ PiperWASMProvider                │     │
│    │  → checkAvailability()           │     │
│    │    ✅ WebAssembly supported      │     │
│    │  → initialize()                  │     │
│    │    → import('piper-tts-wasm')    │     │
│    │    → piperModule.initialize()    │     │
│    └──────────────────────────────────┘     │
│                                              │
│ 2. activeProvider = piperProvider           │
└──────────────────────────────────────────────┘
       │
       ▼
   ✅ App Ready
```

### Nettoyage (App Unmount)

```
App Unmount
   │
   ▼
┌──────────────────────────────────┐
│ ttsProviderManager.dispose()     │
│                                  │
│ for each provider:               │
│   ┌────────────────────────────┐ │
│   │ provider.dispose()         │ │
│   │  → stop()                  │ │
│   │  → clear resources         │ │
│   │  → free WASM memory        │ │
│   └────────────────────────────┘ │
│                                  │
│ activeProvider = null            │
└──────────────────────────────────┘
```

---

## 🧪 Scénarios de Test

### Test 1 : Premier Lancement (Défaut Piper)

```
État Initial:
  localStorage: vide
  IndexedDB: vide

Action:
  1. Ouvrir app
  2. Importer pièce
  3. Cliquer Play

Résultat Attendu:
  ✅ Piper sélectionné par défaut
  ✅ Téléchargement modèle (avec progression)
  ✅ Audio généré et joué
  ✅ Audio mis en cache
  ✅ localStorage: { selectedProvider: 'piper-wasm' }
```

### Test 4 : Assignation de Voix par Genre

```
État Initial:
  Provider: Piper
  Pièce: 4 personnages (2F, 2M)

Action:
  1. PlayDetailScreen → "Voix des personnages"
  2. Définir les genres :
     - JULIETTE → Femme
     - CLAIRE → Femme
     - ROMÉO → Homme
     - MARC → Homme
  3. Lire la pièce

Résultat Attendu:
  ✅ JULIETTE → Voix féminine 1 (ex: Siwis)
  ✅ CLAIRE → Voix féminine 2 (ex: UPMC) - DIFFÉRENTE
  ✅ ROMÉO → Voix masculine 1 (ex: Tom)
  ✅ MARC → Voix masculine 2 (ex: Gilles) - DIFFÉRENTE
  ✅ Relecture → Même assignation (cohérence)
  ✅ Changer JULIETTE en "Homme" → Voix masculine
```

### Test 2 : Changement de Provider

```
État Initial:
  Provider actif: Piper

Action:
  1. Settings → Sélectionner "Natif Device"
  2. Retour à lecture
  3. Cliquer Play

Résultat Attendu:
  ✅ Provider change instantanément
  ✅ Web Speech API utilisé
  ✅ Pas de téléchargement
  ✅ localStorage: { selectedProvider: 'web-speech' }
```

### Test 3 : Cache Hit

```
État Initial:
  Provider: Piper
  Cache: contient audio pour réplique X

Action:
  1. Lire réplique X
  2. Attendre fin
  3. Relire réplique X

Résultat Attendu:
  ✅ 1ère lecture: génération (2s)
  ✅ 2e lecture: cache hit (< 100ms)
  ✅ fromCache = true
  ✅ accessCount incrémenté dans IndexedDB
```

---

## 📊 Métriques et Monitoring

### Points de Mesure

```
┌──────────────────────────────────────┐
│ Performance Metrics                  │
├──────────────────────────────────────┤
│ • Init Time (provider load)          │
│ • Model Download Time                │
│ • Audio Generation Time              │
│ • Cache Hit Rate                     │
│ • Total Storage Used (IndexedDB)     │
│ • Memory Usage (WASM heap)           │
└──────────────────────────────────────┘
```

### Console Logging (Dev)

```typescript
// Dans PiperWASMProvider.synthesize()
console.group('🎙️ Piper Synthesis');
console.log('Text:', text.substring(0, 50) + '...');
console.log('Voice:', options.voiceId);
console.log('Cache Key:', cacheKey);
console.log('Cache Hit:', !!cachedBlob);
console.log('Duration:', duration + 'ms');
console.groupEnd();
```

---

## 🔐 Sécurité et Confidentialité

### Données Sensibles

```
┌─────────────────────────────────────────┐
│ Données Stockées Localement             │
├─────────────────────────────────────────┤
│ ✅ Audio généré (cache)                 │
│    → Blob WAV, pas de texte source      │
│    → Peut être vidé par l'utilisateur   │
│                                         │
│ ✅ Modèles Piper (WASM)                 │
│    → Public, open-source                │
│    → Pas de données personnelles        │
│                                         │
│ ✅ Configuration (localStorage)         │
│    → Provider sélectionné uniquement    │
│    → Pas de données sensibles           │
└─────────────────────────────────────────┘

Aucune donnée envoyée au serveur ✅
Fonctionne 100% hors-ligne ✅
```

---

## 🚀 Déploiement et Build

### Build Pipeline

```
npm run build
   │
   ├─→ TypeScript Compilation
   │     └─→ Check types (strict)
   │
   ├─→ Vite Bundle
   │     ├─→ Minify JS
   │     ├─→ Optimize CSS
   │     └─→ Generate manifest.json
   │
   ├─→ PWA Assets
   │     ├─→ Service Worker
   │     └─→ Icons
   │
   └─→ dist/
       ├─── index.html
       ├─── assets/
       │    ├── index.[hash].js
       │    ├── piper-wasm.[hash].js  ← NOUVEAU
       │    └── ...
       ├─── manifest.json
       └─── sw.js
```

### Service Worker (Cache Strategy)

```
Service Worker

Install:
  → Cache static assets
  → Cache piper-wasm module

Fetch:
  → HTML: Network-first
  → JS/CSS: Cache-first
  → Piper Models: Cache-first (CDN)
  → Audio: IndexedDB (handled by app)
```

---

**Fin des Diagrammes**

Ce document fournit une vue visuelle complète de l'architecture Piper-WASM pour faciliter la compréhension et l'implémentation.